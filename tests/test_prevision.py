...
from services.prevision import get_prevision_base_for_scope, run_prevision
...
def test_run_prevision_monte_carlo(dummy_conn):
    config = PrevisionConfig(horizon_years=3, num_simulations=100, target_goal_amount=120000.0, seed=42)
...
def test_run_prevision_with_dynamic_debt(dummy_conn):
    ...
    with (
        patch("services.vue_ensemble_metrics.get_vue_ensemble_metrics", return_value=fake_metrics),
...
def test_prevision_base_fire_expenses_from_cashflow_mean(conn_with_person):
    ...
    conn.execute("INSERT INTO depenses(person_id, mois, categorie, montant) VALUES (1, '2025-01-01', 'Vie', 100.0)")
    conn.execute("INSERT INTO depenses(person_id, mois, categorie, montant) VALUES (1, '2025-02-01', 'Vie', 300.0)")
    conn.commit()

    with (
        patch("services.vue_ensemble_metrics.get_vue_ensemble_metrics", return_value=fake_metrics),
        patch(
            "services.prevision_base._get_aggregated_debts_schedule",
