...
def _get_fire_annual_expenses(conn, scope_type: str, scope_id) -> float:
    ...
    try:
        ...
        logger.warning("Impossible de calculer fire_annual_expenses pour %s=%s : %s", scope_type, scope_id, exc)
        return 0.0
...
def _get_aggregated_debts_schedule(conn, person_ids: List[int], horizon_years: int = 30) -> pd.Series:
    ...
    """
    from services.credits import get_amortissements, list_credits_by_person
...
def build_prevision_base_for_scope(conn, scope_type: str, scope_id) -> PrevisionBase:
    ...
    if scope_type_lower == "person":
        ...
        if df_family is not None and not df_family.empty:
            ...
            if df_cf is not None and not df_cf.empty:
                last_12 = df_cf.tail(12)
                base_kwargs["current_savings_per_year"] = (
                    float(last_12["savings"].mean() * 12) if not last_12.empty else 0.0
                )
    ...
    base_kwargs["fire_annual_expenses"] = _get_fire_annual_expenses(conn, scope_type_lower, scope_id)
    ...
    # Écriture
    base = PrevisionBase(**base_kwargs, warnings=warnings, metadata=metadata)
