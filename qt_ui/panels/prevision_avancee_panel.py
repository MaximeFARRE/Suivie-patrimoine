        ...
        try:
            ...
            with get_conn() as conn:
                ...
                for preset_key in ("pessimiste", "realiste", "optimiste"):
                    ...
                    results[preset_key] = run_prevision(
                        conn, self._scope_type, self._scope_id,
...
class PrevisionAvanceePanel(QWidget):
    ...
    def _load_stress_scenarios(self):
        try:
            ...
            for key, scenario in list_standard_scenarios().items():
    ...
    def _run_stress(self, config, stress_key: str):
        from services.prevision_stress import list_standard_scenarios
        scenario = list_standard_scenarios().get(stress_key)
        if not scenario:
    ...
    def _on_multi_finished(self, results: dict):
